...
description: >
  多步骤任务规划与执行 — Manus式文件规划（task_plan.md/findings.md/progress.md），
  覆盖：spec→计划（写作plan）、独立session执行（执行plan+review检查点）、
  并行子代理调度。支持/clear后自动恢复。5+ tool calls的复杂任务自动触发。

  触发词: 做计划、规划一下、plan、break down、分解任务、
  多步骤项目、execution plan、subagent、parallel execution。
---
