...
description: >
  学术评估与同行评审 — ScholarEval 框架结构化评分（问题/方法/分析/写作），
  含正式同行评审（CONSORT/STROBE等报告标准检查）和证据质量分级。

  触发词: 评估论文、scholar evaluation、评审论文、peer review、
  审稿、manuscript review、grant review、论文评分。

  文献检索 → literature-review。手稿写作 → scientific-writing。
---
