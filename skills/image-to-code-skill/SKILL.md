...
name: image-to-code
description: >
  图片先行网站设计法 — 先生成高质量设计参考图，深度分析后精确实现为前端代码。
  独立的特殊方法论，不同于其他设计 skill。每段产出独立大图，避免拼板，
  保持 hero 区简洁通透、在笔记本屏幕上可见。

  触发词: image to code、图片转代码、设计稿转代码、截图转网页、
  根据设计图写代码、design to code、image to HTML。

  Trigger (EN): image to code, design to code, screenshot to website,
  convert design to HTML, image-first web implementation.

  通用原型/动画/改版 → huashu-design。轻量着陆页 → design-taste-frontend。
---
