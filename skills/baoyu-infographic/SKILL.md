...
description: >
  信息图与文章配图 — 21种布局×22种视觉风格的专业信息图生成，
  同时覆盖文章插图（Type×Style×Palette三维配图）和封面图（5维度×11调色板×7渲染风格）。
  支持横幅(2.35:1)、宽屏(16:9)、方形(1:1)三种比例。

  触发词: 信息图、infographic、文章配图、为文章配图、illustrate article、
  生成封面图、article cover、make cover、create cover image、
  可视化摘要、visual summary、高密度信息大图。

  SVG技术图表 → baoyu-diagram。知识漫画 → baoyu-comic。
---
