...
description: >
  AI 图像生成 — 多后端聚合（OpenAI GPT Image 2 / Azure / Google / OpenRouter /
  DashScope / Z.AI GLM-Image / MiniMax / Jimeng / Seedream / Replicate / Gemini Web / FLUX）。
  支持 text-to-image、参考图、比例控制、批量生成。默认串行，多prompt时可用并行。

  触发词: 生成图片、AI生图、generate image、create image、图片生成、
  画一张、AI绘画、text to image、image generation。

  网页设计参考图 → imagegen-frontend-web。移动App设计图 → imagegen-frontend-mobile。
---
