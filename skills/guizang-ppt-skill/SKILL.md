...
description: >
  ...
  提供两种风格：① "电子杂志×电子墨水"（衬线+流体背景+暖色）② "瑞士国际主义"（无衬线+网格点阵+IKB/柠檬黄/柠檬绿/安全橙高亮）。
  支持 AI 生成 slide 图片内嵌展示。

  触发词 (中文): 网页PPT、杂志风PPT、瑞士风PPT、Swiss Style、horizontal swipe deck、
  横向翻页演示、HTML演示文稿、发布会PPT、slide deck images、生成slide图片。

  Trigger (EN): web PPT, horizontal swipe deck, Swiss Style slides, magazine style PPT,
  HTML presentation, editorial deck, generate slide images, slide deck images.

  传统 .pptx 文件操作 → pptx skill。学术slides/海报 → scientific-slides。
version: "1.0.0"
---
