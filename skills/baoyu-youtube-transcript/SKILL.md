...
description: >
  下载 YouTube 视频字幕/逐字稿和封面图。通过 URL 或 video ID 获取，
  支持多语言、翻译、章节、说话人识别。缓存原始数据以便快速重新格式化。

  触发词 (中文): YouTube字幕、油管字幕、YouTube逐字稿、下载YouTube字幕、
  获取YouTube封面、视频封面下载、YouTube视频转文字、YouTube文字稿、
  YouTube transcript download、get captions。

  Trigger (EN): YouTube transcript, YouTube subtitles, download YouTube captions,
  YouTube cover image, video thumbnail, get YouTube transcript, YouTube CC,
  extract YouTube subtitles, YouTube video to text, YouTube caption download.
version: 1.1.0
metadata:
  openclaw:
