...
name: parallel-web
description: >
  学术与通用网页搜索 — 聚合 parallel-cli (通用搜索+深度研究+数据增强)、
  Exa (学术语义搜索) 和 Perplexity sonar-pro (学术论文)，自动路由最佳后端。
  覆盖: 网页搜索、URL抓取、批量数据增强、深度研究报告、学术论文检索、引用验证。

  触发词 (中文): 搜索、查一下、网页搜索、学术搜索、找论文、fetch URL、
  提取网页内容、深度研究、deep research、信息增强、查资料。

  Trigger (EN): web search, look up, fetch page, academic search,
  find papers, deep research, data enrichment, extract article,
  scholarly search, research lookup, Exa search, Perplexity search。

  浏览器操作（登录/动态页面）→ web-access。
---
